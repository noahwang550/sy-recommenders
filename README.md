...
From a local clone (editable, pick one extra):
...
Or install directly from the GitHub repo (no clone needed):
...
> **PyPI note**: `recommenders-ai` is not yet on PyPI. Until it is, install from
> the git URL (above) or a GitHub Release wheel. Once published, `pip install recommenders-ai`
> will work and the commands above can switch back to the bare name.

> **Note**: `numpy<2` is pinned in `pyproject.toml` because recommenders v1.2.1 uses `np.NaN` (removed in NumPy 2.0). Any install path above resolves this automatically.
...
下载 `.whl` 后 `pip install <file>` 即可。

> **PyPI 说明**：`recommenders-ai` 尚未发布到 PyPI。在发布之前，请用上面的 git URL 或 GitHub Release wheel 安装。发布后 `pip install recommenders-ai` 即可生效，上述命令也可改回裸包名。

> **注意**：`pyproject.toml` 已锁定 `numpy<2`，因为 recommenders v1.2.1 使用了 `np.NaN`（NumPy 2.0 已移除）。上述任一安装方式都会自动解析此约束。
